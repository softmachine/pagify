class Pagify::CategoriesController < ApplicationController
  ...
  # GET /categories.json
  def index
    @categories = Pagify::Category.all
  ...
  def create
    ...
    respond_to do |format|
      if @category.save
        format.html { redirect_to @category, notice: 'Pagify::Category was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @category.update_attributes(params[:pagify_category])
        format.html { redirect_to @category, notice: 'Pagify::Category was successfully updated.' }
        format.json { head :ok }
      else
